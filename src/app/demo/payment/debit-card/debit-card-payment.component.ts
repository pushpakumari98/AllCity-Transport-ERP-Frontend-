...
export class DebitCardPaymentComponent implements OnInit {
  ...
  ngOnInit() {
    if (history.state && history.state.saleData) {
      ...
      this.paymentMode = history.state.paymentMode || 'DEBIT_CARD';
  ...
  onSubmit() {
    if (this.debitCardForm.valid) {
      this.snackBar.open('Processing debit card payment...', 'Close', {
        duration: 2000
      });

      setTimeout(() => {
        this.snackBar.open('Debit card payment successful!', 'Close', {
          duration: 3000,
